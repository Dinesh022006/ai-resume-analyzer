...
import { CURRENT_AI_MODEL, FALLBACK_AI_MODEL } from "./constants";
...
${resumeText}`;
    ...
    try {
      ...
      console.warn("Primary model gemini-2.5-flash failed:", error);
      const errString = String(error) || "";
      const err = error as Record<string, unknown>;
      const status = err?.status || (err?.response as Record<string, unknown>)?.status;
      
      const isModelError = status === 429 || status === 503 || 
                           errString.includes("429") || errString.includes("503") || 
                           errString.includes("RESOURCE_EXHAUSTED") || errString.includes("quota") || 
                           errString.includes("Too Many Requests") || errString.includes("UNAVAILABLE") || 
                           errString.includes("Service Unavailable") || errString.includes("overload");
